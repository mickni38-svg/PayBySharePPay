...
interface OrderCardVM {
  ...
  /** Deltagerens eget belÃÂ¸b (null = ingen bestilling endnu) */
...
export class OrdersComponent implements OnInit, OnDestroy {
  ...
  private load(): void {
    ...
    this.orderService.getOrdersByParticipant(this.auth.currentUserId() ?? 0).subscribe({
      next: (list) => {
        ...
        // Kun ÃÂ¥bn igangvÃÂ¦rende ordre som default (ikke afsluttede)
  ...
  private buildVM(o: OrderSummaryApiDto): OrderCardVM {
    ...
    // TÃÂ¦l deltagere der har bestilt (OrderSubmitted) eller betalt (Paid)
    ...
    // Alle har bestilt nÃÂ¥r ordre er ReadyToPay eller Completed
    ...
    // Hent detaljer fra cache hvis tilgÃÂ¦ngeligt
    const cached = this._detailsCache().get(o.id);

    // canShowOrderLines: vis linjer sÃÂ¥ snart de er tilgÃÂ¦ngelige (bestilling indsendt)
    ...
    // Deltagerens eget belÃÂ¸b baseret pÃÂ¥ egne ordrelinjer
    ...
    // BelÃÂ¸b der er betalt: sum af ordrelinjer for deltagere med hasPaid = true
    const paidAmount = cached?.participantOrderLines
      .filter(g => g.hasPaid)
      .reduce((sum, g) => sum + g.lines.reduce((s, l) => s + l.lineTotal, 0), 0) ?? 0;

    // Samlet bestilt belÃÂ¸b: sum af alle ordrelinjer uanset betalingsstatus
  ...
  payOrder(vm: { id: number; totalOrderedAmount: number }): void {
    ...
    this.orderService.payOrder(vm.id, userId, vm.totalOrderedAmount).subscribe({
      next: (result) => {
        ...
        // Flyt til Afsluttede fane og genindlÃÂ¦s
      ...
      error: (err) => {
        this.payingOrderId.set(null);
        this.payError.set(
          err.status === 402 ? 'Betaling afvist Ã¢â¬â prÃÂ¸v igen.' :
          err.status === 403 ? 'Kun vÃÂ¦rten kan betale.' :
  ...
  categoryIcon(cat?: string): string {
    const map: Record<string, string> = {
      sushi: 'Ã°Å¸ÂÂ£', pizza: 'Ã°Å¸Ââ¢', burger: 'Ã°Å¸Ââ', drinks: 'Ã°Å¸ÂÂº',
      tacos: 'Ã°Å¸ÅÂ®', ramen: 'Ã°Å¸ÂÅ', kebab: 'Ã°Å¸Â¥â¢', chicken: 'Ã°Å¸Ââ',
      salad: 'Ã°Å¸Â¥â', dessert: 'Ã°Å¸ÂÂ°', coffee: 'Ã¢Ëâ¢', other: 'Ã°Å¸âÂ¦'
    };
    return map[cat ?? ''] ?? 'Ã°Å¸ÂÂ´';
  }
